...
### Performance issue:
Because widget creation is time consuming, I tried the lazzy loading method that does not load all widgets at once but bit by bit, also most of the screen are loaded if user enter in it, so the app have a decent first opening time ( around 5s). You might notice some delay when you enter a screen the first time, this delay is almost 0 when entering the second time.
